 ...
 *
 * 也可以被用于内部bean定义的编程式注册。如果你不关心BeanNameAware和类似的东西，
 * 注册RootBeanDefinition或ChildBeanDefinition是足够了。
 *
...
public class BeanDefinitionHolder implements BeanMetadataElement {
	 ...
	 *
	 * 拷贝构造器:创建一个新的带有和给定的BeanDefinitionHolder实例相同内容的BeanDefinitionHolder。
	 * 注意:包裹的BeanDefinition引用被认为as-is;
	 * 它不是深度拷贝。
	 ...
	 * 判断给定的候选名称是否匹配bean名称或存储在这个bean定义中的别名。
	 */
	 ...
	 * 返回一个bean的长描述，包含名称和别名，也有包含的BeanDefinition的描述。
	 ...
	 * @see #getShortDescription()
	 *
	 * 这个实现返回长描述。可以被覆盖来返回短描述或任何自定义的描述。
	 */
	@Override
